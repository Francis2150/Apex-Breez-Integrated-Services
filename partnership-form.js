(function() {
    /* =============================================
       STORAGE KEY & STATE
    ============================================= */
    var SKEY = 'apex_partnership_data';
    var secFromDir = -1;

    // Mapping from partner field names to secretary field data-f attributes
    var DIR_TO_SEC_MAP = {
    ...
    // Mapping from office static field names to partner residential address field names
    ...
    function dirHTML(i, d) {
        ...
        h += '<div class="dyn-card-header"><h3><i class="fa fa-user-tie"></i> Partner ' + (i + 1) + '</h3>';
    ...
    function subHTML(i, d, linked) {
        ...
        if (isLinked) h += '<div class="synced-badge visible"><i class="fa fa-link"></i> Auto-filled from Partner ' + (linked + 1) + '</div>';
    ...
    function reindexCards(containerId) {
        ...
        cards.forEach(function(c, i) {
            ...
            var label = type === 'director' ? 'Partner' : 'Subscriber';
    ...
    /* =============================================
       OFFICE TO PARTNER ADDRESS LOGIC
    ============================================= */
    ...
    function applySecState() {
        ...
        if (secFromDir >= 0) {
            ...
            noteText.textContent = 'Secretary information is auto-filled from Partner ' + (secFromDir + 1) + '. The Qualification field below can still be edited.';
    ...
    /* =============================================
       SUBSCRIBER LOGIC (Rebuild Pattern)
    ============================================= */
    ...
    function attachListeners() {
        ...
        document.querySelectorAll('.share-pct-input').forEach(function(inp) {
            ...
            newInp.addEventListener('input', function() { updateShareTotal(); save(); });
        });
    ...
    document.getElementById('partnershipForm').addEventListener('submit', function(e) {
        ...
        if (shareTotalEl && shareTotalEl.style.display === 'flex' && shareTotalEl.classList.contains('over')) {
            showToast('Total share allocation must equal 100%.', 'error'); return;
        }
        ...
        .then(function(r) {
            if (r.status === 'success') {
                ...
                updateShareTotal(); attachListeners();
    ...
    function collectFormData() {
        var data = { partnership: {}, office: {}, directors: [], secretary: {}, subscribers: [] };
        ...
        document.querySelectorAll('.static-field').forEach(function(inp) {
            var f = inp.dataset.f;
            if (f.startsWith('off_')) {
                data.office[f.replace('off_', '')] = inp.value;
            } else if (!f.startsWith('sec_')) {
                data.partnership[f] = inp.value;
            }
        });
